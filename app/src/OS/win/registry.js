...
let backgroundKeyPath =
  "HKCU\\Software\\Classes\\directory\\background\\shell\\meView"
...
export function installContextMenu(callback) {
  ...
  function installFileHandler(callback) {
    let appPath = format("%s\\shell\\open\\command", applicationsKeyPath)
    let penarg = format('%s "%1"', execPath)
    args = [appPath, "/ve", "/d", penarg]
